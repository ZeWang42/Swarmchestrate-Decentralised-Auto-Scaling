...
def ewma_update(
    ...
    """Smooth a noisy observed value using EWMA.

    alpha close to 1.0 reacts faster; alpha close to 0.0 is smoother.
    """
    if not valid_number(observed_value) or observed_value <= 0:
        return previous_value if previous_value is not None else observed_value

    if previous_value is None or not valid_number(previous_value) or previous_value <= 0:
        return observed_value

    alpha = max(0.0, min(1.0, alpha))
    return alpha * observed_value + (1.0 - alpha) * previous_value


def estimate_local_service_time(
    deployment: str,
    node_type: str,
    downstreams: list[str],
    lambda_rps: float,
    ...
    """Estimate local service time safely.

    Returns:
        observed_local_service_s: local service-time estimate before EWMA smoothing
        downstream_cost_s: downstream latency removed from observed transit time
        confidence: simple 0..1 confidence score for the decomposition
        method: description for logging/debugging

    The key safety changes are:
    - use a hybrid unknown-fanout estimator instead of assuming serial or parallel calls;
    - cap downstream subtraction so downstream cost cannot dominate total latency;
    - blend decomposed estimate with p50 to avoid unstable near-zero service time;
    - smooth the result outside this function using EWMA.
    """
    min_service_time_s = float(os.getenv("MIN_SERVICE_TIME_MS", "5")) / 1000.0
    max_subtract_ratio = float(os.getenv("DOWNSTREAM_SUBTRACT_CAP_RATIO", "0.7"))
    p50_blend_weight = float(os.getenv("P50_SERVICE_BLEND_WEIGHT", "0.5"))
    # Unknown fanout: we normally do not know whether downstream calls are
    # serial, parallel, async, cached, retried, or partially overlapped.
    # Therefore, avoid exposing a fanout_mode branch and use a conservative
    # hybrid estimator by default.
    unknown_fanout_extra_ratio = float(os.getenv("UNKNOWN_FANOUT_EXTRA_RATIO", "0.3"))

    # Leaf service has no internal downstream path to subtract.
    if node_type == "leaf" or lambda_rps <= 0:
        return max(min_service_time_s, r_p50_s), 0.0, 1.0, "leaf_or_no_load_p50"

    http_rpm_mesh_as_src = http_rpm_mesh_as_src or {}
    grpc_rpm_mesh_as_src = grpc_rpm_mesh_as_src or {}
    http_latency_mesh_avg_as_src = http_latency_mesh_avg_as_src or {}
    grpc_latency_mesh_avg_as_src = grpc_latency_mesh_avg_as_src or {}

    weighted_downstream_latencies: list[float] = []
    total_downstream_rps = 0.0

    logging.info("\n[[DEBUG] Safe local service-time decomposition]]")

    for d in downstreams:
        if d in EXTERNAL_UPSTREAMS:
            continue

        d_rpm = (http_rpm_mesh_as_src.get(d, 0.0) or 0.0) + (grpc_rpm_mesh_as_src.get(d, 0.0) or 0.0)
        d_rps = d_rpm / 60.0

        d_avg_ms = (http_latency_mesh_avg_as_src.get(d, 0.0) or 0.0) + (grpc_latency_mesh_avg_as_src.get(d, 0.0) or 0.0)
        d_avg_s = d_avg_ms / 1000.0

        if d_rps <= 0 or d_avg_s <= 0:
            continue

        requests_per_task = d_rps / max(lambda_rps, 1e-9)
        weighted_cost_s = requests_per_task * d_avg_s
        weighted_downstream_latencies.append(weighted_cost_s)
        total_downstream_rps += d_rps

        logging.info(
            "[DECOMP] downstream=%s d_rps=%.3f req/s avg=%.2f ms calls_per_inbound=%.3f weighted_cost=%.2f ms",
            d, d_rps, d_avg_ms, requests_per_task, weighted_cost_s * 1000.0,
        )

    if not weighted_downstream_latencies:
        return max(min_service_time_s, r_p50_s), 0.0, 0.6, "no_observed_downstream_p50"

    # Unknown fanout estimator:
    # - sum_cost_s assumes all downstream calls are serial, which often over-subtracts.
    # - max_cost_s assumes perfect parallelism, which often under-subtracts.
    # - hybrid uses the critical path plus a fraction of the remaining branches.
    #
    # Formula:
    #   hybrid = max_cost + extra_ratio * (sum_cost - max_cost)
    #
    # With the default extra_ratio=0.3, this means:
    #   critical path + 30% of the non-critical downstream latency.
    sum_cost_s = sum(weighted_downstream_latencies)
    max_cost_s = max(weighted_downstream_latencies)
    unknown_fanout_extra_ratio = max(0.0, min(1.0, unknown_fanout_extra_ratio))

    raw_downstream_cost_s = max_cost_s + unknown_fanout_extra_ratio * (sum_cost_s - max_cost_s)
    raw_downstream_cost_s = min(raw_downstream_cost_s, sum_cost_s)
    method = f"hybrid_unknown_fanout_{unknown_fanout_extra_ratio:.2f}"

    max_subtract_s = max_subtract_ratio * max(r_avg_s, min_service_time_s)
    safe_downstream_cost_s = min(raw_downstream_cost_s, max_subtract_s)

    decomposed_service_s = max(min_service_time_s, r_avg_s - safe_downstream_cost_s)
    observed_local_service_s = (
        p50_blend_weight * max(min_service_time_s, r_p50_s)
        + (1.0 - p50_blend_weight) * decomposed_service_s
    )

    coverage_ratio = total_downstream_rps / max(lambda_rps, 1e-9)
    subtract_ratio = safe_downstream_cost_s / max(r_avg_s, min_service_time_s)

    # Simple confidence heuristic; useful for logging and later fallback rules.
    confidence = 1.0
    if coverage_ratio < 0.2 or coverage_ratio > 5.0:
        confidence -= 0.3
    if subtract_ratio > max_subtract_ratio:
        confidence -= 0.3
    if raw_downstream_cost_s > max_subtract_s:
        confidence -= 0.2
    confidence = max(0.0, min(1.0, confidence))

    logging.info(
        "[DECOMP SUMMARY] deployment=%s method=%s raw_downstream=%.2f ms safe_downstream=%.2f ms "
        "R_avg=%.2f ms R_p50=%.2f ms observed_S_local=%.2f ms coverage=%.2f subtract_ratio=%.2f confidence=%.2f",
        deployment, method, raw_downstream_cost_s * 1000.0, safe_downstream_cost_s * 1000.0,
        r_avg_s * 1000.0, r_p50_s * 1000.0, observed_local_service_s * 1000.0,
        coverage_ratio, subtract_ratio, confidence,
    )

    return observed_local_service_s, safe_downstream_cost_s, confidence, method
...
def das_loop(
    ...
    # EWMA state for local service time. This must live outside the loop so each
    # iteration can reuse the previous smoothed estimate.
    previous_s_local_s: float | None = None
    ...
    while True:
        try:
            ...
            # Estimate local execution-only service time safely.
            #
            # Previous approach:
            #   S_local = R_avg - sum(weighted downstream latency)
            #
            # That is fragile when downstream calls are parallel, retried, cached,
            # async, or partially overlapped. The new approach uses a hybrid
            # unknown-fanout downstream estimate, caps subtraction, blends with P50,
            # and then smooths the result with EWMA.
            S_local_observed_s, downstream_cost_s, decomp_confidence, decomp_method = estimate_local_service_time(
            ...
            service_time_ewma_alpha = float(os.getenv("SERVICE_TIME_EWMA_ALPHA", "0.3"))
            S_local_s = ewma_update(
                observed_value=S_local_observed_s,
                previous_value=previous_s_local_s,
                alpha=service_time_ewma_alpha,
            )
            previous_s_local_s = S_local_s

            logging.info(
                "[EWMA SERVICE TIME] observed_S_local=%.2f ms | smoothed_S_local=%.2f ms | alpha=%.2f | method=%s | confidence=%.2f",
                S_local_observed_s * 1000.0, S_local_s * 1000.0, service_time_ewma_alpha,
                decomp_method, decomp_confidence,
            )

            # Local structural state/thread lock contention handling.
            # Keep this after EWMA so contention response is also less noisy.
            if R_avg_s > (3.0 * max(R_p50_s, 0.001)):
