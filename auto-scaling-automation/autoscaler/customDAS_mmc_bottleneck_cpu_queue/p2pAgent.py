...
class P2PAgent:
    ...
    def handle_frontend_slo_violation(self, peer_id: str, message: dict[str, Any]):
        """Receive frontend/root SLO violation broadcasts.

        This does not mean this service is definitely the bottleneck. It is a
        global pressure signal: every downstream service should compare itself
        against its own healthy-P95 guideline and run the M/M/c recommendation
        if needed.
        """
        logging.info("[P2P EVENT] Received FRONTEND SLO VIOLATION from %s: %s", peer_id, message)

        payload = {
            "type": "MSG_FRONTEND_SLO_VIOLATION",
            "from": peer_id,
            "timestamp": message.get("timestamp", time.time()),
            "data": message,
        }
        ...
        with self._inbox_lock:
            self._message_inbox.append(payload)
    ...
    def send_bottleneck_alert(
        ...
        observed_tail_ms: float | None = None,
        tail_label: str = "P95",
        reason: str = "",
        extra: dict[str, Any] | None = None,
    ) -> bool:
        """Send a structured bottleneck alert to a downstream autoscaling agent.

        The DAS loop uses this message to trigger recursive bottleneck diagnosis.
        The downstream receives an allowed selected-tail latency budget from its
        upstream. The selected tail is described by ``tail_label`` (for example
        ``P90`` or ``P95``), and the budget is always carried in the generic
        ``allowed_tail_ms`` field.
        """
        normalized_tail_label = (tail_label or "P95").strip().upper()

        payload: dict[str, Any] = {
            "from": self.peer_id,
            "you_are_bottleneck": True,
            "tail_label": normalized_tail_label,
            "allowed_tail_ms": float(allowed_tail_ms),
            "reason": reason,
            "timestamp": time.time(),
        }

        if observed_tail_ms is not None:
            payload["observed_tail_ms"] = float(observed_tail_ms)

        if extra:
            # Allow callers to add metadata, but do not reintroduce percentile-
            # specific protocol fields such as allowed_p95_ms.
            for key, value in extra.items():
                if key in {"allowed_p95_ms", "observed_p95_ms", "allowed_p90_ms", "observed_p90_ms"}:
                    continue
                payload[key] = value

        return self.send_message(target, "MSG_BOTTLENECK_ALERT", payload)

    def send_frontend_slo_violation(
        self,
        target: str,
        frontend_tail_ms: float,
        frontend_slo_ms: float,
        tail_label: str = "P95",
        reason: str = "frontend_slo_violation_broadcast",
        extra: dict[str, Any] | None = None,
    ) -> bool:
        """Send a frontend/root SLO violation broadcast to one downstream peer."""
        normalized_tail_label = (tail_label or "P95").strip().upper()
        ...
        return self.send_message(target, "MSG_FRONTEND_SLO_VIOLATION", payload)

    def drain_pressure_messages(self) -> list[dict[str, Any]]:
        """Return only messages that should trigger reactive DAS pressure handling."""
    ...
    def drain_bottleneck_alerts(self) -> list[dict[str, Any]]:
        """Return only bottleneck-alert messages from the inbox.

        This is optional convenience for DAS.py. get_messages() is still kept
        for general use.
        """
        messages = self.get_messages()
        return [msg for msg in messages if msg.get("type") == "MSG_BOTTLENECK_ALERT"]
    ...
    def init_peer(self):
        ...
        self.peer.register_message_handler("MSG_FRONTEND_SLO_VIOLATION", self.handle_frontend_slo_violation)
