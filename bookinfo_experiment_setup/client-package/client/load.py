...
class Experiment:
    ...
    def name(self) -> str:
        if self.hpa_mode == "none":
            return f"bookinfo_{self.workload_name}_hpa_none"
        return f"bookinfo_{self.workload_name}_hpa_cpu{self.cpu_target}"
...
def run_locust(exp: Experiment) -> dict[str, str]:
    ...
    return {
        ...
        "stats_csv": f"{prefix}_{timestamp}_stats.csv",
        "failures_csv": f"{prefix}_{timestamp}_failures.csv",
        "exceptions_csv": f"{prefix}_{timestamp}_exceptions.csv",
        "stats_history_csv": f"{prefix}_{timestamp}_stats_history.csv",
    }
