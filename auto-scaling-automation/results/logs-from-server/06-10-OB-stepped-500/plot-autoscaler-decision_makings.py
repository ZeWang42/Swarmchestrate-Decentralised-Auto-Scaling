...
DEFAULT_ORDER = ["Dadqn", "HPA-80", "Custom-DAS", "DAS"]
...
def load_data(data_dir: Path, files: dict[str, list[str]]) -> pd.DataFrame:
    frames = []
    ...
    for scheme, file_list in files.items():
        for run_id, filename in enumerate(file_list, start=1):
            ...
            df = df.dropna(subset=["Timestamp", "Name", "Pods"])
            ...
            df["Minute"] = (df["ElapsedSec"] // 60).astype(int)
...
def calculate_decision_counts(
    ...
    per_minute = (
        ...
        .sort_values(["Scheme", "Run", "Name", "Minute"])
    )
    ...
    per_run_service = (
        ...
        .sum()
        .rename(columns={"Decision": "DecisionCount"})
    )
...
def plot_by_microservice(
    ...
    pivot = pivot[existing_order + remaining]

    ax = pivot.plot(
        kind="bar",
        figsize=(13, 6),
        width=0.82,
        color=[SCHEME_COLORS.get(col, DEFAULT_COLOR) for col in pivot.columns],
    )

    ax.set_xlabel("Microservice")
    ax.set_ylabel("Decision count")
    ax.set_title("Decision makings per microservice")
...
def plot_total(
    ...
    bars = plt.bar(
        ...
        color=[SCHEME_COLORS.get(scheme, DEFAULT_COLOR) for scheme in totals["Scheme"]],
    )
    ...
    plt.title("Total decision makings per autoscaler")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Plot autoscaler decision-making counts "
            "from deployment pod CSV logs."
        )
    )
    ...
    by_service, totals = calculate_decision_counts(
        data,
        tolerance=args.tolerance,
    )

    by_service.to_csv(
        args.out_dir / "decision_counts_by_microservice.csv",
        index=False,
    )

    totals.to_csv(
        args.out_dir / "decision_counts_total.csv",
        index=False,
    )
    ...
    plot_by_microservice(
        by_service,
        DEFAULT_ORDER,
        args.out_dir / "decision_counts_by_microservice.png",
    )

    plot_total(
        totals,
        DEFAULT_ORDER,
        args.out_dir / "decision_counts_total.png",
    )

    print("Saved:")
    print(args.out_dir / "decision_counts_by_microservice.csv")
    print(args.out_dir / "decision_counts_total.csv")
    print(args.out_dir / "decision_counts_by_microservice.png")
    print(args.out_dir / "decision_counts_total.png")
