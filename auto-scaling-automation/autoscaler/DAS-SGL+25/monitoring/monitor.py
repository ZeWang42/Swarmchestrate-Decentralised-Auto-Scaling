...
from monitoring.istio_metrics import (
    check_prometheus,
    get_grpc_latency_mesh,
    get_grpc_latency_p95,
    get_grpc_latency_p95_between,
    get_grpc_rpm,
    get_grpc_rpm_between,
    get_grpc_rpm_mesh,
    get_http_latency_mesh,
    get_http_latency_p95,
...
class Monitor:
    ...
    # ------------------------------------------------------------------
    # Istio mesh views grouped by source workload
    # destination_workload is fixed, values are broken down by source
    # ------------------------------------------------------------------

    def get_http_rpm_mesh(self, destination_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_http_rpm_mesh(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )
