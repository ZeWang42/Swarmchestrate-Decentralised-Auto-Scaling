...
from monitoring.istio_metrics import (
    ...
    get_http_latency_p10_as_src,
    get_grpc_latency_p10_as_src,
    get_http_latency_p10_as_dst,
    get_grpc_latency_p10_as_dst,
    get_http_latency_p10_between,
    get_grpc_latency_p10_between,
    get_http_latency_p10_mesh_as_dst,
    get_grpc_latency_p10_mesh_as_dst,
    get_http_latency_p10_mesh_as_src,
    get_grpc_latency_p10_mesh_as_src,
    ...
    get_http_latency_p25_as_src,
    get_grpc_latency_p25_as_src,
    get_grpc_latency_p50_as_src,
    get_http_latency_p50_as_src,
    get_grpc_latency_p90_as_src,
    get_http_latency_p90_as_src,
    ...
    get_http_latency_p25_as_dst,
    get_grpc_latency_p25_as_dst,
    get_grpc_latency_p50_as_dst,
    get_http_latency_p50_as_dst,
    get_grpc_latency_p90_as_dst,
    get_http_latency_p90_as_dst,
    get_http_latency_p95_as_dst,
    get_grpc_latency_p95_as_dst,

    get_http_rpm_between,
    get_grpc_rpm_between,
    get_http_latency_p25_between,
    get_grpc_latency_p25_between,
    ...
    get_http_latency_p25_mesh_as_dst,
    get_grpc_latency_p25_mesh_as_dst,
    get_http_latency_p90_mesh_as_dst,
    get_http_latency_p95_mesh_as_dst,
    get_grpc_latency_p90_mesh_as_dst,
    ...
    get_http_latency_p25_mesh_as_src,
    get_grpc_latency_p25_mesh_as_src,
    get_http_latency_p90_mesh_as_src,
    get_http_latency_p95_mesh_as_src,
    get_grpc_latency_p90_mesh_as_src,
...
class Monitor:
    ...
    def get_http_latency_p25_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p25_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p25_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p25_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p50_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p50_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p50_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p50_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_http_latency_p90_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p90_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p90_as_src(self, source_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p90_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p25_as_dst(self, destination_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p25_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p25_as_dst(self, destination_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p25_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p90_as_dst(self, destination_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p90_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p90_as_dst(self, destination_workload: str) -> float:
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p90_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p25(self, deployment_name: str) -> float:
        return self.get_http_latency_p25_as_dst(deployment_name)

    def get_grpc_latency_p25(self, deployment_name: str) -> float:
        return self.get_grpc_latency_p25_as_dst(deployment_name)
    ...
    def get_http_latency_p25_between(
        ...
        if not self.prometheus_available:
            return 0.0
        return get_http_latency_p25_between(
            self.prom_url,
            source_workload,
            destination_workload,
            self.prometheus_period,
    ...
    def get_grpc_latency_p25_between(
        ...
        if not self.prometheus_available:
            return 0.0
        return get_grpc_latency_p25_between(
            self.prom_url,
            source_workload,
            destination_workload,
            self.prometheus_period,
    ...
    def get_http_latency_p25_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_http_latency_p25_mesh_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p25_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_grpc_latency_p25_mesh_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )

    def get_http_latency_p90_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_http_latency_p90_mesh_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )
    ...
    def get_grpc_latency_p90_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_grpc_latency_p90_mesh_as_dst(
            self.prom_url,
            destination_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p25_mesh_as_src(self, source_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_http_latency_p25_mesh_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_grpc_latency_p25_mesh_as_src(self, source_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_grpc_latency_p25_mesh_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )

    def get_http_latency_p90_mesh_as_src(self, source_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_http_latency_p90_mesh_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )
    ...
    def get_grpc_latency_p90_mesh_as_src(self, source_workload: str) -> dict[str, float]:
        if not self.prometheus_available:
            return {}
        return get_grpc_latency_p90_mesh_as_src(
            self.prom_url,
            source_workload,
            self.prometheus_period,
        )
    ...
    def get_http_latency_p25_mesh(self, destination_workload: str) -> dict[str, float]:
        return self.get_http_latency_p25_mesh_as_dst(destination_workload)

    def get_grpc_latency_p25_mesh(self, destination_workload: str) -> dict[str, float]:
        return self.get_grpc_latency_p25_mesh_as_dst(destination_workload)
...
# ----------------------------------------------------------------------
# P10 latency methods added dynamically for backward-compatible Monitor API
# ----------------------------------------------------------------------

def _monitor_get_http_latency_p10_as_src(self, source_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_http_latency_p10_as_src(self.prom_url, source_workload, self.prometheus_period)


def _monitor_get_grpc_latency_p10_as_src(self, source_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_grpc_latency_p10_as_src(self.prom_url, source_workload, self.prometheus_period)


def _monitor_get_http_latency_p10_as_dst(self, destination_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_http_latency_p10_as_dst(self.prom_url, destination_workload, self.prometheus_period)


def _monitor_get_grpc_latency_p10_as_dst(self, destination_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_grpc_latency_p10_as_dst(self.prom_url, destination_workload, self.prometheus_period)


def _monitor_get_http_latency_p10_between(self, source_workload: str, destination_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_http_latency_p10_between(self.prom_url, source_workload, destination_workload, self.prometheus_period)


def _monitor_get_grpc_latency_p10_between(self, source_workload: str, destination_workload: str) -> float:
    if not self.prometheus_available:
        return 0.0
    return get_grpc_latency_p10_between(self.prom_url, source_workload, destination_workload, self.prometheus_period)


def _monitor_get_http_latency_p10_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
    if not self.prometheus_available:
        return {}
    return get_http_latency_p10_mesh_as_dst(self.prom_url, destination_workload, self.prometheus_period)


def _monitor_get_grpc_latency_p10_mesh_as_dst(self, destination_workload: str) -> dict[str, float]:
    if not self.prometheus_available:
        return {}
    return get_grpc_latency_p10_mesh_as_dst(self.prom_url, destination_workload, self.prometheus_period)


def _monitor_get_http_latency_p10_mesh_as_src(self, source_workload: str) -> dict[str, float]:
    if not self.prometheus_available:
        return {}
    return get_http_latency_p10_mesh_as_src(self.prom_url, source_workload, self.prometheus_period)


def _monitor_get_grpc_latency_p10_mesh_as_src(self, source_workload: str) -> dict[str, float]:
    if not self.prometheus_available:
        return {}
    return get_grpc_latency_p10_mesh_as_src(self.prom_url, source_workload, self.prometheus_period)


Monitor.get_http_latency_p10_as_src = _monitor_get_http_latency_p10_as_src
Monitor.get_grpc_latency_p10_as_src = _monitor_get_grpc_latency_p10_as_src
Monitor.get_http_latency_p10_as_dst = _monitor_get_http_latency_p10_as_dst
Monitor.get_grpc_latency_p10_as_dst = _monitor_get_grpc_latency_p10_as_dst
Monitor.get_http_latency_p10_between = _monitor_get_http_latency_p10_between
Monitor.get_grpc_latency_p10_between = _monitor_get_grpc_latency_p10_between
Monitor.get_http_latency_p10_mesh_as_dst = _monitor_get_http_latency_p10_mesh_as_dst
Monitor.get_grpc_latency_p10_mesh_as_dst = _monitor_get_grpc_latency_p10_mesh_as_dst
Monitor.get_http_latency_p10_mesh_as_src = _monitor_get_http_latency_p10_mesh_as_src
Monitor.get_grpc_latency_p10_mesh_as_src = _monitor_get_grpc_latency_p10_mesh_as_src

# Backward-compatible aggregate and mesh aliases.
Monitor.get_http_latency_p10 = lambda self, deployment_name: self.get_http_latency_p10_as_dst(deployment_name)
Monitor.get_grpc_latency_p10 = lambda self, deployment_name: self.get_grpc_latency_p10_as_dst(deployment_name)
Monitor.get_http_latency_p10_mesh = lambda self, destination_workload: self.get_http_latency_p10_mesh_as_dst(destination_workload)
Monitor.get_grpc_latency_p10_mesh = lambda self, destination_workload: self.get_grpc_latency_p10_mesh_as_dst(destination_workload)
