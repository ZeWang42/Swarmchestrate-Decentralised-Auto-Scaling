...
class ExperimentSetupRequest(BaseModel):
    ...
    #workload_name: int = Field(description="Client-side load parameter, e.g. 100/300/500")
...
def experiment_setup(req: ExperimentSetupRequest) -> ExperimentSetupResponse:
    ...
    # Ze-TODO: Before experiment starts, reset state, it should delete hpa and ensure all replicas as 1

# Ze: reset system state BEFORE experiment
    try:
    # 1. delete existing HPA (if any)
        _delete_hpa_for_application(
            app_name=BOOKINFO_APP_NAME,
            namespace=req.namespace,
            deployment_names=BOOKINFO_DEPLOYMENTS,
        )
    except Exception as e:
        # don't fail experiment if HPA doesn't exist
        pass

    # 2. scale all deployments to 1 replica
    apps_api = client.AppsV1Api()

    for dep in BOOKINFO_DEPLOYMENTS:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to scale {dep}: {str(e)}")

    # 3. wait until all replicas are ready
    import time
    ...
    while True:
        ...
        for dep in BOOKINFO_DEPLOYMENTS:
            d = apps_api.read_namespaced_deployment(dep, req.namespace)
        ...
        if time.time() - start > timeout:
            raise HTTPException(status_code=500, detail="Timeout waiting for deployments to scale to 1")
    ...
    monitor_resp = start_monitor(
        StartMonitorRequest(
            ...
            file_prefix=f"{req.monitor.file_prefix}_{req.app}_rps{req.workload_name}_hpa{req.hpa.mode}"
