...
def main() -> None:
    ...
    x_labels = [
        X_LABELS[name]
        for name in summary_df["Autoscaler"]
    ]

    # Keep the original bar color coding unchanged.
    bars = ax.bar(
        x,
    ...
    # -----------------------------------------------------------------
    # Individual experiment runs: black dots, matching plot_slo(3).py
    # -----------------------------------------------------------------
    ...
    autoscaler_positions = {
        autoscaler: i
        for i, autoscaler
        in enumerate(
            summary_df["Autoscaler"]
        )
    }

    for autoscaler in summary_df[
        "Autoscaler"
    ]:
