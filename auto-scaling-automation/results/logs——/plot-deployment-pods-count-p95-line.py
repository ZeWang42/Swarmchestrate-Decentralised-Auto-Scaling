...
for scheme in files.keys():
    ...
    ax1.set_ylabel("#Pods", color=colors["Pods"])
    ax1.tick_params(axis="y", labelcolor=colors["Pods"])
    ax1.set_ylim(0, 40)     # Pods axis
    ...
    ax2.set_ylim(0, 2000)    # Latency axis


    plt.title(f"{scheme}: pod count and p95 latency per minute")

    fig.tight_layout()
    plt.savefig(f"{scheme.lower().replace('-', '_')}_pods_p95_latency.pdf", dpi=600)
    plt.show()
