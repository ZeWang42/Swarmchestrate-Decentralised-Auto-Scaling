...
def get_configured_latency_slo_ms(app_name: str, deployment: str, *, allow_env_override: bool = True) -> float:
    """Resolve the fixed latency SLO (ms) for a deployment.

    Env var overrides only apply to the root/frontend service; every other
    deployment must load its fixed SLO from the per-app baseline table.
    """
...
def healthy_percentile_target_ms(history_ms: list[float]) -> float | None:
    ...
    """
    return percentile(history_ms, 0.25)  # 25th percentile of healthy self tail latencies


def update_healthy_latency_history(
    history_ms: list[float],
    observed_local_latency_ms: float,
    #observed_frontend_latency_ms: float,
    target_frontend_latency_ms: float,
    max_samples: int = 40,
) -> bool:
    if not valid_number(observed_local_latency_ms) or observed_local_latency_ms <= 0:
        return False
    if not valid_number(target_frontend_latency_ms) or target_frontend_latency_ms <= 0:
        return False
    ...
    if observed_local_latency_ms >= target_frontend_latency_ms:
        history_ms.append(float(target_frontend_latency_ms))
    else:
        history_ms.append(float(observed_local_latency_ms))
    if len(history_ms) > max_samples:
        del history_ms[: len(history_ms) - max_samples]
    return True
...
# grpc_latency_p90_mesh_as_src = monitor.get_grpc_latency_p90_mesh_as_src(deployment)
            ...
            # The root/frontend service is the only one whose SLO may be overridden via env
            # vars; every other deployment always resolves its fixed SLO from the baseline table.
            is_root_service = deployment in ROOT_SERVICES
            APP_NAME = os.getenv("APP_NAME", "onlineboutique").strip().lower()
            configured_latency_slo_ms = get_configured_latency_slo_ms(
                APP_NAME, deployment, allow_env_override=is_root_service
            )
            logging.info(
            ...
            if effective_latency_slo_mode == "adaptive":
                ...
                if learned_latency_limit_ms is not None and valid_number(learned_latency_limit_ms):
                    ...
                    effective_latency_target_ms = configured_latency_slo_ms
                # Frontend health must be judged against the frontend's own SLO, not this service's.
                frontend_healthy_threshold_ms = get_configured_latency_slo_ms(APP_NAME, ROOT_SERVICE)
                if valid_number(endpoint_frontend_latency_ms) and frontend_healthy_threshold_ms > 0 and endpoint_frontend_latency_ms < 0.7*frontend_healthy_threshold_ms:
...
def main() -> None:
    ...
    scale_down_cooldown = 120.0  # Ze-TODO: for testing, should be removed
